...
**Decision (2025-06):** pno-table ships **light theme only** for now. Dark mode will be added later.

When working on UI:

- Keep the design **flat and minimal** — no gradients, glow effects, backdrop-blur hero backgrounds, or decorative overlays.
- Use **one brand accent** (`--brand`, teal) for primary buttons, links, active nav/tabs/sidebar items, and focus rings. Map via Tailwind: `text-brand`, `bg-brand-subtle`, `ring-brand/25`. Do not add secondary accent colors or gradients.
- Use **visible structure** — borders, `bg-muted` bands, and white `bg-card` panels to separate regions (header, sidebar, content, tables). Avoid an all-white layout where sections blend together.
- Use **shadcn semantic tokens** (`background`, `foreground`, `muted`, `accent`, `border`, etc.) and Tailwind utilities (`text-foreground`, `bg-muted`, `border-border`) instead of bespoke palette names.
- Legacy CSS variables (`--sea-ink`, `--lagoon`, `--surface`, …) in `src/styles.css` are aliases to shadcn tokens; prefer semantic classes in new code.
